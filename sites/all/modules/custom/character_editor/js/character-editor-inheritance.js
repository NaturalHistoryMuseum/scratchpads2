(function($){
  Drupal.CharacterInheritance = function(){
    ...
    this.contextMenu = function(info){
      ...
      var elements = [];
      elements.push({element: Drupal.t('Values'), subtitle: true});
      elements.push({element: Drupal.t('get from descendants'), callback: $.proxy(this, 'contextClickCallback', info, metadata, 'up'), css: {backgroundImage: pass_up ? selected_background : '', backgroundRepeat: 'no-repeat', backgroundPosition: '0 center'}});
      elements.push({element: Drupal.t('pass down to descendants'), callback: $.proxy(this, 'contextClickCallback', info, metadata, 'down'), css: {backgroundImage: pass_down ? selected_background : '', backgroundRepeat: 'no-repeat', backgroundPosition: '0 center'}});
      return elements;
    }
    ...
    this.contextClickCallback = function(info, metadata, type){
      ...
      for( var i = 0; i < rows.length; i++) {
        ...
        // And send the data to be saved ; this may be different per row so we
        // have to do them one by one
        slickgrid.callback('update', {entity_ids: [data.id], column_id: info.column.id, pass: cell_metadata.pass, flag: cell_metadata.flag, plugin: 'CharacterMetadata'});
      }
    }
    ...
    this.updateCell = function(metadata, node){
      ...
      $(node).css({backgroundImage: image, backgroundRepeat: 'no-repeat', backgroundPosition: '100% 0'});
    }
