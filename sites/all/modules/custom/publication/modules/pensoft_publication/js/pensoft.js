     ...
     attach: function(context) {
       ...
       function insert_references(inserts, settings) {
         ...
         if ($warn.length == 0 && warning.length > 0) {
           $warn = $('<div class="pensoft_add_multiple_references_warning warning"><p>' +
               Drupal.t('The following references have been added inline and will be available once the publication is saved:') +
               '<p><ul></ul></div>').prependTo($('#edit-group_publication_references div.fieldset-content'));
         }
         
         for (var i = 0; i < warning.length; i++) {
           $('ul', $warn).append('<li class="pensoft_add_multiple_references">' + warning[i] + '</li>');
         }
       }
       ...
       function insert_figures(inserts, settings) {
         ...
         for (var i = 0; i < inserts.length; i++) {
           // Get fid
           //var matches = inserts[i].match(/insert_from_view_file_(\d+)([^\d]|$)/);
           var matches = inserts[i].match(/^(\d+):(.+)$/);
           if (matches != null && matches.length > 0) {
             var fid = matches[1];
         ...
         if ($warn.length == 0 && warning.length > 0) {
           $warn = $('<div class="pensoft_add_multiple_figures_warning warning"><p>' +
               Drupal.t('The following figures have been added inline and will be available once the publication is saved:') +
               '<p><ul></ul></div>').prependTo($('#edit-group_publication_figures div.fieldset-content'));
         }
         
         for (var i = 0; i < warning.length; i++) {
           $('ul', $warn).append('<li class="pensoft_add_multiple_figures">' + warning[i] + '</li>');
         }
      ...
      function init() {
        ...
        $('div.form-type-textarea textarea:not(.pensoft_processed)', context).bind('insertFromView-insert', function(event, inserts, settings) {
