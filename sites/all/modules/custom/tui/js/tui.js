(function($){
  ...
  Drupal.tui.resize_frame = function(){
    window.setTimeout(function(){
      ...
      if($('#tui-tree-subcontainer ol').height() > ($(window).height() - height_offset)) {
        $('#tui-tree-subcontainer').css('height',
            ($(window).height() - height_offset) + 'px');
  ...
  Drupal.tui.scrollto = function(element){
    $('#tui-tree-subcontainer').animate(
        {
          scrollTop: $(element).position().top
              + $('#tui-tree-subcontainer').scrollTop() - 50
        }, 500);
  }
  $(document).ready(function(){
    // First load, check if we have one term highlighted, if so, load the form.
  ...
  Drupal.behaviors.tui = {
    attach: function(context, settings){
                ...
                switch($(this).attr('id')){
                  case 'tui-delete':
                    ...
                    if($('.tui-highlight').length) {
                      ...
                      $('#tui-tree-links').after(
                          '<div id="tui-tree-form"></div>');
                      var element_settings = {};
                      element_settings.progress = {
                        'type': 'throbber'
                      };
                      element_settings.url = $('#tui-delete').data('url')
                          + tids;
                      element_settings.event = 'click';
                      Drupal.ajax['tui-click'] = new Drupal.ajax('tui-click',
                          $('#tui-click'), element_settings);
                      $('#tui-click').click();
                    }
                    break;
                  case 'tui-search':
                    ...
                    $('#tui-tree-form').remove();
                    $('#tui-tree-links')
                        .after('<div id="tui-tree-form"></div>');
                    var element_settings = {};
                    element_settings.progress = {
                      'type': 'throbber'
                    };
      ...
      $('li.tui-has-children>div>span', context).bind(
          'click',
          function(e){
            if($(this).parent().parent().hasClass('tui-closed')
                && $(this).parent().parent().hasClass('tui-never-opened')) {
              ...
              $.getJSON(Drupal.settings.tui.callback + "/"
                ...
                Drupal.attachBehaviors($(
                    'li[data-tui-child-of="'
                        + clicked_term.parent().parent().data('tui-this-term')
                        + '"]').parent().parent());
                if(clicked_term.parent().hasClass('tui-highlight')) {
                  $('div', clicked_term.parent().parent()).addClass(
                      'tui-highlight');
                }
              });
      ...
      // RESIZABLE
      $('#tui-tree', context).resizable(
          {
            handles: 'e',
            resize: function(event, ui){
              $('#tui-form').width(
                  $('#tui').width() - ($('#tui-tree').width() + 2));
            },
            minWidth: 250
          });
      // RESIZE WINDOW
      ...
      // SORTING
      $('#tui-tree-subcontainer>ol').nestedSortable(
          {
            cursorAt: {
              top: 15
            },
            ...
            update: function(event, ui){
              if($('li[data-tui-child-of="' + ui.item.data('tui-child-of')
                  + '"]').length == 1) {
                $(
                    'li[data-tui-this-term="' + ui.item.data('tui-child-of')
                        + '"]').removeClass('tui-open');
                $(
                    'li[data-tui-this-term="' + ui.item.data('tui-child-of')
                        + '"]').removeClass('tui-has-children');
                $(
                    'li[data-tui-this-term="' + ui.item.data('tui-child-of')
                        + '"]').addClass('tui-no-children');
              }
              if(ui.item.parent().parent().data('tui-this-term')) {
                ui.item.attr('data-tui-child-of', ui.item.parent().parent()
                    .data('tui-this-term'));
                ui.item.data('tui-child-of', ui.item.parent().parent().data(
                    'tui-this-term'));
              } else {
