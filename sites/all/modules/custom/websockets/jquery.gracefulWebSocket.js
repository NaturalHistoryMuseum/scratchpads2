 ...
 * 
 * If browser supports WebSockets a native WebSocket instance is returned. If
 * not, a simulated half-duplex implementation is returned which uses polling
 * over HTTP to retrieve new messages
 * 
 * OPTIONS
 * -----------------------------------------------------------------------------
 * 
 * {Number} fallbackOpenDelay number of ms to delay simulated open event for
 * fallback {Number} fallbackPollInterval number of ms between requests for
 * fallback polling {Object} fallbackPollParams optional params to pass with
 * each poll requests
 * 
 * EXAMPLES
 * -----------------------------------------------------------------------------
 ...
 * 
 * var websocket = $.gracefulWebSocket({ fallbackPollParams: {
...
(function($){
  ...
  $.extend({
    ...
    gracefulWebSocket: function(url, options){
      ...
      // Default properties
      this.defaults = {
        keepAlive: false, // not implemented - should ping server to keep socket
                          // open
        autoReconnect: false, // not implemented - should try to reconnect
                              // silently if socket is closed
        fallback: true, // not implemented - always use HTTP fallback if native
