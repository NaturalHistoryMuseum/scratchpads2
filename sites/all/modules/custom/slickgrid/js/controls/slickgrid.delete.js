...
(function($){
  function SlickGridDelete(grid, $container){
    ...
    function confirmDialog(){
      ...
      $("<button>Delete</button>").click(doDelete).appendTo($dialog);
      $("<button>Cancel</button>").click(cancelDelete).appendTo($dialog);
      slickgrid.openDialog($container, $dialog);
    }
    function doDelete(){
      slickgrid.closeDialog();
      var data = {entity_ids: slickgrid.getEntityIDs(), entity_type: options['entity_type']}
