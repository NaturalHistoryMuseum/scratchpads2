...
(function($){
  function SlickGridUndo($container){
    ...
    function confirmDialog(){
      ...
      $("<button>Undo</button>").click(doUndo).appendTo($dialog);
      $("<button>Cancel</button>").click(cancelUndo).appendTo($dialog);
    ...
    function doUndo(){
      ...
      if(command && Slick.GlobalEditorLock.cancelCurrentEdit()) {
        // Undo is handled via revisions so post to backend all entities updated
        // as part of this command
        // Get the column
        var c = grid.getColumns()[command.cell];
        var data = {display_id: slickgrid.getViewDisplayID(), view: slickgrid.getViewName(), field_name: c.field, field_id: c.id, updated: command.updated}
    ...
    $.extend(this, {
    // Methods
    "queueAndExecuteCommand": queueAndExecuteCommand, "queueCommand": queueCommand});
    init();
  }
  // Slick.Controls.Undo
